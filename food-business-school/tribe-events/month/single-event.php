...
$additional_data = array();
if ( $cpi === '' ) {
	$additional_data['courseLink'] = tribe_get_event_link( $post );
} else {
	$additional_data['courseLink'] = get_post_permalink( $cpi );
}
...
<div id="tribe-events-event-<?php echo $event_id ?>" class="<?php tribe_events_event_classes() ?>" data-tribejson='<?php echo tribe_events_template_data( $post, $additional_data ); ?>'>
	<h3 class="tribe-events-month-event-title entry-title summary"><a href="<?php print $additional_data['courseLink'] ?>" class="url"><?php the_title() ?></a></h3>
